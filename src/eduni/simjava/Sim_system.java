 ...
 * Instantiated entities are added to <code>Sim_system</code> which is
 * responsible for synchronising their behaviour. Time progresses on the basis
 * of event scheduling times. These events are held in two queues, instances of
 * <code>Evqueue</code>, the future queue and the deferred queue. All the
 * runtime functionality available to entities is managed by
 * <code>Sim_system</code>.
 * <p>
 * In the previous versions of <code>SimJava</code>, users would use
 * <code>Sim_system</code> only to initialise the simulation, add the entities,
 * link their ports, and finally run the simulation. In version 2.0
 * <code>Sim_system</code> makes a lot more functionality available to the
 * modeller.
 * <p>
 * Three new aspects of <code>Sim_system</code> are the definition of a
 * transient condition, a termination condition, and the selection of an output
 * analysis method to apply. All these are managed internally by
 * <code>Sim_system</code> so the user is not required to include relevant code
 * in the simulation's entities. To set the <b>transient condition</b> for the
 * simulation the user uses a <code>set_transient_condition</code> method. The
 * simulation's <b>termination condition</b> is set by using a
 * <code>set_termination_condition</code> method. Finally, an <b>output analysis
 * method</b> is selected with one of the <code>set_output_analysis</code>
 * methods.
 * <p>
 * Available transient conditions are:
 * <ul>
 * <li>
 * A number of event completions. In this case the simulation is considered to
 * have entered steady state after a certain number of event service completions
 * at a given entity. Event completions are signalled to <code>Sim_system</code>
 * with the entities'
 * <code><a href="Sim_entity.html#sim_completed(eduni.simjava.Sim_event)">sim_completed</a></code>
 * method.
 * <li>
 * A specific period of simulated time. After this time period elapses the
 * simulation is considered to have entered steady state.
 * <li>
 * The minimum-maximum method. This method allows <code>Sim_system</code> to try
 * and identify the transient period based on the minimum-maximum method for a
 * given measure. This method considers steady state to have been entered once a
 * measure observation has been located which is neither the minimum nor the
 * maximum of the remaining observations.
 * <li>
 ...
 * <ul>
 * <li>
 * A number of event completions. The simulation is terminated once a given
 * number of events complete service at a given entity. This condition is
 * similar to the relevant transient condition.
 * <li>
 * A specific period of simulated time. After this time period elapses the
 * simulation is terminated.
 * <li>
 * A measure's confidence interval accuracy. This method allows
 * <code>Sim_system</code> to run the simulation until a confidence interval for
 * a given measure has reached a certain accuracy level. To use this method, the
 * user needs to provide the measure of interest, an output analysis method
 * which will be used as a variance reduction technique, the confidence level,
 * and the desired accuracy. The accuracy is the ratio of the confidence
 * interval's halfwidth over the measure's total mean.
 * <li>
 * None. As before this is the default if no condition is specified. Non
 * terminating simulations have meaning only for animation purposes in which
 * case a simulation is used as an elaborate demo of a system.
 * </ul>
 * <p>
 * Entities may use the <code>running</code> method to check whether the
 * simulation has completed.
 * <p>
 * Finally, the available output analysis methods are:
 * <ul>
 * <li>
 * Independent replications. In this case, the simulation run is repeated a
 * number of times to produce total measurements.
 * <li>
 * Batch means. With this method, a single, long run is performed and then the
 * obtained observations are batched into batches and their means are then used
 * to produce the total measurements. The number of batches used is the one that
 * gives the least serial correlation between the batch means. for the majority
 * of measures.
 * <li>
 * None. Again this is the default method. In this case no output analysis is
 * performed and only available sample measurements are produced.
 * </ul>
 * <p>
 * Since version 2.0 of <code>SimJava</code>, <code>Sim_system</code> also
 * allows the modeller greater control over the <b>seeding</b> of the random
 * number generators. The simulation's generators can be created in such a way
 * to allow <code>Sim_system</code> to automatically seed them, ensuring well
 * spaced sample sequences for the generators. The original seed to be used as
 * well as the seed spacing desired can be set with the
 * <code>set_seed_sequence</code> method. If this method is not used, the
 * default seed of 4851 is used to produce seeds spaced by 100000 samples. If
 * independent replications are used as an output analysis method and reseeding
 * of the generators is desired in each subsequent replication, the generators
 * need to be added to their entities. This is accomplished with the entities'
 * <code><a href="Sim_entity.html#add_generator(eduni.simjava.distributions.Generator)">add_generator</a></code>
 * method. Otherwise the previous replication's last seed is used as the next
 * one's initial seed.
 * <p>
 * <code>Sim_system</code> also provides control over the simulation's output.
 * The <b>trace file</b> (<code>sim_trace</code>) can be managed with more
 * detail. The <code>set_trace_detail</code> method can be used to specify
 * whether or not to add the default trace , trace generated by the entities, or
 * trace for specific event types. In this last case, the event tags of interest
 * can be supplied to <code>Sim_system</code> with a <code>track_event</code>
 * method. The level of entity trace included in the trace file can be managed
 * with the <code>set_trace_level</code> method. Finally, tracing can be
 * switched off alltogether with the <code>set_auto_trace</code> method. The
 * default is not to produce a trace file.
 * <p>
 * Another major aspect of the simulation's output is the <b>simulation's
 * report</b>. The report is automatically produced by <code>Sim_system</code>
 * and includes information on the simulation depending on the specified
 * conditions and parameters. The detail level of the report file (
 * <code>sim_report</code>) can be modified with the
 * <code>set_report_detail</code> method. The default information included are
 * the total obtained measurements obtained as well as general information about
 * the simulation run. In the case of no output analysis method having been
 * used, the steady state sample measurements are presented in place of the
 * total measurements. If an output analysis method has been used,
 * <code>set_report_detail</code> can be used to include the sample measurements
 * for each replication (for independent replications), or the sample
 * measurements for each batch (for batch means). Finally, the original seeds
 * used in the random number generators of the simulation can also be included
 * in the report. Since version 2.0, the simulation's report is also available
 * to animated simulations.
 * <p>
 * A <code>generate_graphs</code> method may also be used to generate
 * <b>graphs</b> for the simulation. Graphs are generated for all detailed
 * measures and present the progress of each measure's sample average over the
 * progress of the simulation. Several additional information may also be
 * displayed for each graph such as the mean confidence interval, the transient
 * period, the batches etc. The simulation will produce a <i>".sjg"</i> file
 * containing the simulation data. This file may then be opened with the
 * <code>SimJava Graph Viewer</code>, a graph viewing utility for
 * <code>SimJava</code> simulations.
 * <p>
 * More information on the definition of simulations and the use of
 * <code>Sim_system</code> can be found at the <a
 * href="http://www.dcs.ed.ac.uk/home/simjava/tutorial/index.html#2">SimJava
 * Tutorial</a>.
 * 
 * @see Sim_entity
...
public class Sim_system {
	...
	// The types of termination and transient conditions, and output analysis
	// methods
	...
	/**
	 * A constant representing the minimum-maximum method for transient period
	 * identification.
	 */
	public static final int MIN_MAX = 3;
	/**
	 * A constant representing the output analysis method of independent
	 * replications.
	 */
	...
	/**
	 * A constant representing a termination condition based on a confidence
	 * interval's accuray.
	 */
	public static final int INTERVAL_ACCURACY = 6;

	// Private fields used to store the termination condition parameters
	private static int term_condition = NONE; // The termination condition to be
												// used
	private static int term_entity_id; // The id of the entity containing the
										// measure on which the termination
										// condition is based
	private static int term_event_type; // The event tag of the events to be
										// considered for the termination
										// condition
	private static double term_time = 0.0; // The termination time
	private static double initial_term_time = 0.0; // The termination time
													// speficied by the user
	private static long initial_term_count = 0; // The count of event
												// completions specified by the
												// user for the termination
												// condition
	private static long term_count = 0; // The count of event completions for
										// the termination condition
	private static int[] term_count_bm; // The count of event completions for a
										// termination condition using batch
										// means
	private static List term_times; // Stores the termination times in the case
									// of the minimum-maximum method being used
	private static long term_event_counter = 0; // The counter for termination
												// condtion event completions
	private static long transient_term_event_count = 0; // The count of
														// termination events
														// completed during the
														// transient period
	private static String term_measure; // The measure used in the termination
										// condition
	private static double term_accuracy; // The accuracy for the confidence
											// interval used in the termination
											// condition

	// Private fields used to store the termination comdition parameters
	private static int trans_condition = NONE; // The transient condition being
												// used
	private static int trans_entity_id; // The id of the entity containing the
										// measure on which the transient
										// condition is based
	private static int trans_event_type; // The event tag of events to be
											// considered for the transient
											// condition
	private static double trans_time = -1.0; // The transient time
	private static String trans_measure; // The measure on which the transient
											// conditnion is based
	private static double initial_trans_time = 0.0; // The transient time
													// specified by the user
	private static long trans_count = 0; // The count of events for the
											// transient period estimation
	private static long initial_trans_count = 0; // The count of transient
													// events for the transient
													// period condition
													// specified by the user
	private static long trans_event_counter = 0; // The counter for events that
													// count towards the
													// transient condition

	private static boolean include_transient; // Flag for specifying whether the
												// transient condition should be
												// considered a part of the
												// termination condition or not
	private static boolean in_steady_state = false; // Flag for checking whether
													// the simulation is in
													// steady state

	// Fields concerning output analysis methods
	private static int output_analysis_type = NONE; // The output analysis
													// method being used
	private static boolean incomplete = true; // Flag for telling whether a
												// simulation is complete or not
	private static int replication_count = 10; // The default number of
												// replications to be made
	private static double confidence_level = 0.90; // The default confidence
													// level
	private static List replications; // Stores the data from each replication
	private static double total_time_elapsed = 0.0; // The total simulated time
													// elapsed in all
													// replications
	private static double total_transient_time = 0.0; // The total transient
														// time elapsed in all
														// replications
	private static int min_batches = 10; // The default minimum number of
											// batches
	private static int max_batches = 20; // The default maximum number of
											// batches

	private static Object[] run_data; // Contains all the data obtained from the
										// run

	// Fields for the management of the trace file
	private static int[] trace_tags; // The tags for which trace will be
										// generated
	private static boolean default_trace = true, // Flag for including or not
													// the default trace
			entity_trace = true, // Flag for including or not the entity trace
			event_trace = true; // Flag for including or not the event specific
								// trace

	// Private data members
	private static List entities; // The current entity list
	private static List backup; // A backup containing the entities in their
								// initial state
	...
	private static boolean running; // Flag for checking if the simulation is
									// running
	private static Semaphore onestopped; // Semaphore for synchronising entities
	private static Semaphore onecompleted; // Semaphore for registering entities
											// having completed
	private static Sim_output trcout; // The output object for trace messages
	private static int trace_level; // The trace level for which event trace
									// will be generated
	...
	private static HashMap wait_predicates; // The predicates used in entity
											// wait methods

	// Fields concerning the simulation's report
	private static long start_date; // The actual time at which the simulation
									// started
	private static boolean detailed_report = false; // The default is to produce
													// a report that isn't
													// detailed
	private static boolean include_seeds = false; // The default is to produce a
	...
	private static Sim_random_obj seed_source = new Sim_random_obj(
			"Seed generator", root_seed); // The seed generator
	private static boolean not_sampled = true; // Flag that is checked to see
												// whether the seed generator
												// has been sampled

	private static boolean anim_stopped = false; // Flag used to check whether
													// (in animated versions)
													// the user has clicked on
													// the stop button

	// Fields concerning graph generation
	private static final String suffix = ".sjg"; // The suffix for SimJava
													// graphs
	private static boolean generate_graphs = false; // Flag used to check
													// whether or not to
													// generate graphs
	private static String graph_file = "sim_graphs" + suffix; // The default
																// graph file to
																// be used

	private static boolean efficient_measure_defined = false; // Flag used to
																// check whether
																// the
																// simulation
																// contains an
	...
	/**
	 * Initialise the simulation for standalone simulations. This function
	...
	/**
	 * Initialise the simulation for animated simulations. This method should
	 * not be directly called by the user.
	 * 
	 * @param out
	 *            The instance of <code>Sim_anim</code> used for generating the
	 *            animation
	 * @param sim
	 ...
	 * 
	 * @param out
	 *            The <code>Sim_output</code> instance that will receive the
	 *            simulation trace
	 * @param sim
	 *            The simulation thread.
	 */
	...
	/**
	 * Get the current simulation time. This method is identical to
	 * <code>sim_clock()</code> and is present for compatibility with existing
	 * simulations.
	 * 
	...
	/**
	 * Get the current trace level (initially <code>0xff</code>), which controls
	 * trace output. This method is identical to <code>get_trace_level()</code>
	 * and is present for compatibility with existing simulations.
	 * 
	 ...
	 * @throws Sim_exception
	 *             If the entity was not found. This error can be left
	 *             unchecked.
	 */
	...
	/**
	 * Set the trace level which controls entity trace output. This method is
	 * identical to <code>set_trace_level()</code> and is present for
	 ...
	 * 
	 * @param on
	 *            If <code>true</code> then the messages are switched on, if
	 *            <code>false</code> they are switched off.
	 */
	...
	public static void add(Sim_entity e) {
		...
		if (running) {
			...
			evt = new Sim_event(Sim_event.CREATE, clock,
					current_ent().get_id(), 0, 0, e);
	 ...
	 * @param port1
	 *            The name of the port on the first entity
	 * @param ent2
	 *            The name of the second entity
	 * @param port2
	 *            The name of the port on the second entity
	 */
	public static void link_ports(String ent1, String port1, String ent2,
			String port2) {
		Sim_port p1, p2;
		Sim_entity e1, e2;
		e1 = get_entity(ent1);
	...
	static synchronized void send(int src, int dest, double delay, int tag,
			Object data) {
		if (delay < 0.0) {
			throw new Sim_exception("Sim_system: Send delay can't be negative.");
		}
		Sim_event e = new Sim_event(Sim_event.SEND, clock + delay, src, dest,
				tag, data);
	...
	static synchronized void select(int src, Sim_predicate p) {
		...
		if (found) {
			...
			if (auto_trace
					&& (default_trace || (event_trace && is_trace_tag(ev
							.get_tag())))) {
				trace(src, "selected event type " + ev.get_tag()
						+ " (event time was " + ev.event_time() + ")");
			}
		} else {
	...
	static synchronized void cancel(int src, Sim_predicate p) {
		...
		if (found) {
			...
			if (auto_trace
					&& (default_trace || (event_trace && is_trace_tag(ev
							.get_tag())))) {
				trace(src, "cancelled event type " + ev.get_tag()
						+ " (event time was " + ev.event_time() + ")");
			}
		} else {
			((Sim_entity) entities.get(src)).set_evbuf(null);
	...
	static synchronized void putback(Sim_event ev) {
		deferred.add_event(ev);
	}
	...
	private static void process_event(Sim_event e) {
		...
		case (Sim_event.SEND):
			...
			dest = e.get_dest();
			if (dest < 0) {
				throw new Sim_exception(
						"Sim_system: Attempt to send to a null entity detected.");
			} else {
				int tag = e.get_tag();
				if (((dest != e.get_src()) || (tag != 9999))
						&& ((Sim_entity) entities.get(dest)).has_stat()) {
					((Sim_entity) entities.get(dest)).update(Sim_stat.ARRIVAL,
							tag, clock);
				}
				dest_ent = (Sim_entity) entities.get(dest);
				if (dest_ent.get_state() == Sim_entity.WAITING) {
					Integer destObj = new Integer(dest);
					Sim_predicate p = (Sim_predicate) wait_predicates
							.get(destObj);
	...
	/**
	 * Set the sample generators' seed sequence. If this method is not called
	 * the defaults are 4851 as the root seed and 100000 as the desired sample
	 * spacing.
	 * 
	 * @param seed_specing
	 *            The seed spacing
	 * @param root_seed
	 *            The seed used to generate all the simulation's seeds
	 */
	...
	/**
	 * Set the sample generators' root seed. This is the seed used to generate
	 * the initial seeds for the simulation's sample generators.
	 * 
	 * @param new_seed
	 *            The new root seed
	 */
	...
	/**
	 * Generate the next seed based on the root seed and the seed spacing. This
	 * method is automatically called and doesn't need to be included on user
	 * simulations.
	 * 
	...
	// Called when an event is considered to have completed service. If the
	// event that completed is the one
	...
	static synchronized void job_completed(int entity, int event_tag) {
		...
		if (auto_trace
				&& (default_trace || (event_trace && is_trace_tag(event_tag)))) {
			trace(entity, "event type " + event_tag + " completed service");
		}
	}

	// Called to update the count of collected observations of interest. This is
	// done in the case of
	// a termination condition based on collected observations and interval
	// accuracy.
	static synchronized void observation_collected(int entity, String measure) {
		if ((term_condition == INTERVAL_ACCURACY) && (entity == term_entity_id)
				&& (measure.equals(term_measure)) && in_steady_state) {
			term_event_counter++;
		}
	}

	/**
	 * Internal method used to initialise the simulation. This should <b>not</b>
	 * be called from user simulations.
	 */
	public static void run_initialise() {
		...
		if (output_analysis_type == IND_REPLICATIONS) {
			// Make a backup of the entities. This is done in order to perform
			...
			for (int i = 0; i < entities_size; i++) {
				...
				try {
					...
					throw new Sim_exception("Could not make backup of entity "
							+ ent.get_name() + ".");
				}
			}
		}
		...
		for (int i = 0; i < entities_size; i++) {
			...
			for (int j = 0; j < generators_size; j++) {
				...
				gens.add(new Object[] { gen.get_name(),
						new Long(gen.get_seed()) });
			}
	...
	/**
	 * Specify a termination condition based on event completions at a specific
	 * entity.
	 * 
	 * @param type
	 *            The termination condition type. This must be set to
	 *            <code>EVENTS_COMPLETED</code>.
	 * @param entity
	 *            The name of the entity whose events will be used for the
	 *            termination condition
	 * @param event_type
	 *            The event tag of the events to be counted towards the
	 *            termination condition
	 * @param count
	 *            The event completion count that when reached, will satisfy the
	 *            termination condition
	 * @param include_transient
	 *            <code>true</code> if the number of completions provided should
	 *            only apply to completions in steady state. <code>false</code>
	 *            if they should apply to the entire run length.
	 */
	public static void set_termination_condition(int type, String entity,
			int event_type, long count, boolean include_transient) {
		if (type != EVENTS_COMPLETED) {
			throw new Sim_exception(
					"The method used must be passed a condition based on completed events.");
		} else if (count <= 0) {
			throw new Sim_exception(
					"The count for events completed must be positive.");
		} else if (term_condition != NONE) {
			throw new Sim_exception(
					"You can't specify two termination conditions.");
		}
		term_condition = type;
	 ...
	 * @param type
	 *            The termination condition type. This must be set to
	 *            <code>TIME_ELAPSED</code>.
	 * @param time
	 *            The simulated time after which the termination condition will
	 *            be satisfied
	 * @param include_transient
	 *            <code>true</code> if the condition's time period should be
	 *            considered only after steady state has been reached,
	 *            <code>false</code> if it should be considered from the
	 *            beginning of the simulation.
	 */
	public static void set_termination_condition(int type, double time,
			boolean include_transient) {
		if (type != TIME_ELAPSED) {
			throw new Sim_exception(
					"The method used must be passed a condition based on the elapsed simulation time.");
		} else if (time <= 0.0) {
			throw new Sim_exception(
					"The simulation termination time completed must be positive.");
		} else if (term_condition != NONE) {
			throw new Sim_exception(
					"You can't specify two termination conditions.");
		}
	...
	/**
	 * Specify a termination condition based on the confidence interval accuracy
	 * obtained for a custom measure. This condition is considered only once the
	 * transient period has elapsed. The accuracy is the ratio of the confidence
	 * interval's halfwidth over the measure's total mean.
	 * 
	 * @param type
	 *            The termination condition type. This must be set to
	 *            <code>INTERVAL_ACCURACY</code>.
	 * @param output_analysis_type
	 *            The output analysis method to be used as a variance reduction
	 *            technique
	 * @param level
	 *            The confidence level for which the confidence interval will be
	 *            calculated
	 * @param accuracy
	 *            The accuracy that is required to satisfy the termination
	 *            condition
	 * @param entity
	 *            The name of the entity that contains the measure upon which
	 *            the termination condition is based
	 * @param measure
	 *            The name of the custom measure upon which the termination
	 *            condition is based
	 */
	public static void set_termination_condition(int type,
			int output_analysis_type, double level, double accuracy,
			String entity, String measure) {
		if (type != INTERVAL_ACCURACY) {
			throw new Sim_exception(
					"The method used must be passed a condition based on the accuracy of a confidence interval.");
		} else if ((output_analysis_type != IND_REPLICATIONS)
				&& (output_analysis_type != BATCH_MEANS)) {
			throw new Sim_exception(
					"The output analysis method to be used must be either IND_REPLICATIONS or BATCH_MEANS.");
		} else if ((level <= 0.0) || (level >= 1.0)) {
			throw new Sim_exception(
					"The confidence level must be between 0.0 and 1.0.");
		} else if (accuracy <= 0.0) {
			throw new Sim_exception(
					"The interval accuracy tolerance must be greater than 0.0.");
		} else if (Sim_system.output_analysis_type != NONE) {
			throw new Sim_exception(
					"When using this termination condition no other output analysis method can be specified.");
		} else if (trans_condition == MIN_MAX) {
			throw new Sim_exception(
					"This termination condition can't be used when the minimum-maximum method is used.");
		} else if (term_condition != NONE) {
			throw new Sim_exception(
					"You can't specify two termination conditions.");
		} else if ((output_analysis_type == BATCH_MEANS)
				&& (efficient_measure_defined)) {
			throw new Sim_exception(
					"Batch means may not be used when efficient measures are defined.");
		}
		term_entity_id = get_entity_id(entity);
		Sim_stat stat = ((Sim_entity) entities.get(term_entity_id)).get_stat();
		if (stat == null) {
			throw new Sim_exception("No measures have been defined for "
					+ entity + ".");
		}
		int measure_type = stat.get_type(measure);
		if (measure_type == -1) {
			throw new Sim_exception(measure + " has not been defined for "
					+ entity + ".");
		}
		term_condition = type;
	...
	/**
	 * Specify a termination condition based on the confidence interval accuracy
	 * obtained for a default measure. This condition is considered only once
	 * the transient period has elapsed. The accuracy is the ratio of the
	 * confidence interval's halfwidth over the measure's total mean.
	 * 
	 * @param type
	 *            The termination condition type. This must be set to
	 *            <code>INTERVAL_ACCURACY</code>.
	 ...
	 *            the termination condition is based
	 * @param measure
	 *            The <code>int</code> constant that represents the default
	 *            measure upon which the termination condition is based
	 */
	public static void set_termination_condition(int type,
			int output_analysis_type, double level, double accuracy,
			String entity, int measure) {
		if (type != INTERVAL_ACCURACY) {
			throw new Sim_exception(
					"The method used must be passed a condition based on the accuracy of a confidence interval.");
		} else if ((output_analysis_type != IND_REPLICATIONS)
		...
		if (measure_type == -1) {
			throw new Sim_exception(measure_name + " has not been defined for "
					+ entity + ".");
		}
		term_condition = type;
	...
	/**
	 * Specify a transient condition using the minimum-maximum method for a
	 * custom measure. The minimum-maximum method considers the simulation to
	 * have entered steady state after the first observation of the given
	 * measure is found that is neither the minimum nor the maximum of the
	 * remaining observations.
	 * 
	 * @param type
	 *            The transient condition type. This must be set to
	 *            <code>MIN_MAX</code>.
	 * @param entity
	 *            The name of the entity that contains the measure upon which
	 *            the minimum-maximum method will be applied
	 * @param measure
	 *            The name of the custom measure upon which the minimum-maximum
	 *            method will be applied
	 */
	public static void set_transient_condition(int type, String entity,
			String measure) {
		if (type != MIN_MAX) {
			throw new Sim_exception(
					"The method used must be passed a condition based on minimum-maximum values of observations of a specific entity's measure.");
		} else if (trans_condition != NONE) {
			throw new Sim_exception(
					"You can't specify two transient conditions.");
		} else if ((term_condition == INTERVAL_ACCURACY)
				|| (term_condition == NONE)) {
			throw new Sim_exception(
					"The minimum-maximum method can only be used with a termination of event completions or elapsed time.");
		} else if (efficient_measure_defined) {
			throw new Sim_exception(
					"The minimum-maximum method can't be used with efficient measures.");
		}
		...
		if (measure_type == -1) {
			throw new Sim_exception(measure + " has not been defined for "
					+ entity + ".");
		} else if (((measure_type != Sim_stat.STATE_BASED) && (measure_type != Sim_stat.INTERVAL_BASED))
				|| (measure.equals("Utilisation"))) {
			throw new Sim_exception(measure + " of " + entity
					+ " has invalid type for a transient period estimator.");
		}
	...
	/**
	 * Specify a transient condition using the minimum-maximum method for a
	 * default measure. The minimum-maximum method considers the simulation to
	 * have entered steady state after the first observation of the given
	 * measure is found that is neither the minimum nor the maximum of the
	 * remaining observations.
	 * 
	 ...
	 * @param measure
	 *            The <code>int</code> constant that represents the default
	 *            measure upon which the minimum-maximum method will be applied
	 */
	public static void set_transient_condition(int type, String entity,
			int measure) {
		if (type != MIN_MAX) {
		...
		if (measure_type == -1) {
			throw new Sim_exception(measure_name + " has not been defined for "
				...
				|| (measure_name.equals("Utilisation"))) {
			throw new Sim_exception(measure_name + " of " + entity
					+ " has invalid type for a transient period estimator.");
		}
	...
	/**
	 * Specify a transient condition based on event completions at a specific
	 * entity.
	 * 
	 * @param type
	 *            The transient condition type. This must be set to
	 *            <code>EVENTS_COMPLETED</code>.
	 * @param entity
	 *            The name of the entity whise event completions will be counted
	 *            towards this condition
	 * @param event_type
	 *            The event tag of the events whose completions will be counted
	 *            towards this condition
	 * @param count
	 *            The number of event completions after which the condition will
	 *            be satisfied
	 */
	public static void set_transient_condition(int type, String entity,
			int event_type, long count) {
		if (type != EVENTS_COMPLETED) {
			throw new Sim_exception(
					"The method used must be passed a condition based on completed events.");
		} else if (count <= 0) {
	 ...
	 *            <code>TIME_ELAPSED</code>.
	 * @param time
	 *            The time period after which the condition will be satisfied
	 */
	...
	/**
	 * Specify an output analysis method for the simulation. The default, if
	 * this method is not used is <code>NONE</code>. If independent replications
	 * is selected, 10 replications will be performed. If batch means is
	 * selected, the observations will be batched into 10 to 20 batches. The
	 * actual number of batches used will be the one that gives the least serial
	 * correlation among the batch means.
	 * <p>
	 * The confidence level that will be used for calculating the simulation's
	 * measures' confidence intervals is <code>0.90</code> (<code>90%</code>).
	 * 
	 * @param type
	 *            The output analysis method
	 */
	public static void set_output_analysis(int type) {
		if ((type != NONE) && (type != IND_REPLICATIONS)
				&& (type != BATCH_MEANS)) {
			throw new Sim_exception(
					"The output analysis method must be either NONE, IND_REPLICATIONS or BATCH_MEANS.");
		} else if (term_condition == INTERVAL_ACCURACY) {
			throw new Sim_exception(
					"No additional output analysis method can be specified when using a termination condition based on an interval accuracy.");
		} else if (output_analysis_type != NONE) {
			throw new Sim_exception(
					"You can't specify two output analysis methods.");
		} else if ((output_analysis_type == BATCH_MEANS)
				&& efficient_measure_defined) {
			throw new Sim_exception(
					"Batch means may not be used when efficient measures are defined.");
	...
	/**
	 * Specify an output analysis method to produce confidence intervals of a
	 * given confidence level.
	 * 
	 * @param type
	 *            The output analysis method.
	 * @param level
	 *            The confidence level to be used to produce the confidence
	 *            intervals for the simulations' measures
	 */
	public static void set_output_analysis(int type, double level) {
		if ((type != IND_REPLICATIONS) && (type != BATCH_MEANS)) {
			throw new Sim_exception(
					"The output analysis method must be either IND_REPLICATIONS or BATCH_MEANS.");
		} else if ((level <= 0.0) || (level >= 1.0)) {
			throw new Sim_exception(
					"The confidence level must be between 0.0 and 1.0.");
		} else if (term_condition == INTERVAL_ACCURACY) {
	...
	/**
	 * Specify batch means as the simulation's output analysis method. The
	 * observation will be batched into <code>min_batches</code> batches up to
	 * <code>max_batches</code> batches. The number of batches that will
	 * actually be used to produce the confidence intervals will be the one that
	 * gives the least serial correlation among the batch means.
	 * 
	 * @param type
	 *            The output analysis method. This must be set to
	 *            <code>BATCH_MEANS</code>.
	 * @param min_batches
	 *            The minimum number of batches that the observations will be
	 *            batched into
	 * @param max_batches
	 *            The maximum number of batches that the observations will be
	 *            batched into
	 * @param level
	 *            The confidence level to be used to produce the confidence
	 *            intervals for the simulations' measures
	 */
	public static void set_output_analysis(int type, int min_batches,
			int max_batches, double level) {
		if (type != BATCH_MEANS) {
			throw new Sim_exception(
					"The output analysis method must be either IND_REPLICATIONS or BATCH_MEANS.");
		} else if ((min_batches <= 1) || (min_batches > max_batches)) {
			throw new Sim_exception(
					"Invalid minimum and maximum number of batches.");
		} else if ((level <= 0.0) || (level >= 1.0)) {
			throw new Sim_exception(
					"The confidence level must be between 0.0 and 1.0.");
		} else if (term_condition == INTERVAL_ACCURACY) {
	 ...
	 * @param type
	 *            The output analysis method. This must be set to
	 *            <code>IND_REPLICATIONS</code>.
	 * @param replications
	 *            The number of replications to perform
	 * @param level
	 *            The confidence level that will be used to produce the
	 *            simulation's measures' confidence intervals
	 */
	public static void set_output_analysis(int type, int replications,
			double level) {
		if (type != IND_REPLICATIONS) {
			throw new Sim_exception(
					"This method may only be used to specify IND_REPLICATIONS as the output analysis method.");
		} else if (replications < 2) {
			throw new Sim_exception(
					"The number of replications must be greater than 2.");
		} else if ((level <= 0.0) || (level >= 1.0)) {
			throw new Sim_exception(
					"The confidence level must be between 0.0 and 1.0.");
		} else if (term_condition == INTERVAL_ACCURACY) {
	...
	/**
	 * Set the detail required in the simulation's report. The default is to
	 * include only the total measurements and the general simulation
	 * information.
	 * 
	 * @param detailed
	 *            If set to <code>true</code> the report will include the sample
	 *            measurements from each replication (if independent
	 *            replications has been used as an output analysis method), or
	 *            the sample measurements from each batch (if batch means has
	 *            been used). Setting this to <code>false</code> will only
	 *            include the total measurements and general simulation
	 *            information.
	 * @param seeds
	 *            If set to <code>true</code> the report will include the
	 *            original seeds used in the entities' random number generators
	 */
	...
	/**
	 * Specify that graphs are to be generated in the default graph file. The
	 * name of the graph file will be <code>sim_graphs.sjg</code>.
	 * 
	 * @param generate_graphs
	 *            Set to <code>true</code> if graphs are to be generated,
	...
	/**
	 * Internal method used to tidy up the entities' <code>Sim_stat</code>
	 * objects. This method should <b>not</b> be used in user simulations.
	 */
	...
	/**
	 * Internal method used to check if the transient or termination conditions
	 * have been satisfied. This method should <b>not</b> be used in user
	 * simulations.
	 * 
	 * @return <code>true</code> if the simulation should keep on running,
	 *         <code>false</code> otherwise
	 */
	public static boolean check_conditions() {
		...
		case EVENTS_COMPLETED:
			...
			case EVENTS_COMPLETED:
				if (include_transient) {
					// Termination event count considered over the whole
					// simulation run
					...
					if (in_steady_state) {
						...
						if (trans_event_counter >= trans_count) {
							...
							// Start counting termination events from the
							// beginning
			...
			case MIN_MAX:
				if (term_event_counter >= term_count) {
					...
					if (include_transient) {
						...
						result = false;
						trans_time = (((Sim_entity) entities
								.get(trans_entity_id)).get_stat())
								.min_max_time(trans_measure);
						if (trans_time != -1.0) {
							in_steady_state = true;
						} else {
							print_message("The termination condition has been satisfied but no transient period has been identified.");
						...
						if (in_steady_state) {
							...
							// The transient period is computed. The run must be
							// extended by the transient time.
							trans_time = (((Sim_entity) entities
									.get(trans_entity_id)).get_stat())
									.min_max_time(trans_measure);
							if (trans_time == -1.0) {
								// The transient period has not yet been
								// overcome.
								double choice = 0.0;
								if (!animation) {
		...
		case TIME_ELAPSED:
			...
			case TIME_ELAPSED:
				...
				break;
			case MIN_MAX:
				// The run length should complete. Following this the transient
				// period is estimated. Depending on
				// whether or not the simulation termination is based on steady
				// state or the whole run, the
		...
		case INTERVAL_ACCURACY:
			// The transient period must elapse before this termination
	...
	private static double prompt_modeller(int type) {
		...
		try {
			...
			case 0:
				...
				System.out
						.println("The transient period could not be estimated. How should the run proceed?");
				System.out
						.println("1. Complete run immediately with no transient period.");
				System.out
						.println("2. Extend the run for a number of event completions and try again.");
				...
				if (!choice.equals("1")) {
					long more_events = 0L;
					System.out
							.println("Event completions for which the run will be extended: ");
					do {
						try {
							more_events = (Long.valueOf(input.readLine()))
									.longValue();
			...
			case 1:
				...
				System.out
						.println("1. Complete run immediately with no transient period.");
				System.out
						.println("2. Extend the run for a time period and try again.");
				System.out.print("Choice: ");
				do {
					choice = input.readLine();
				} while (!choice.equals("1") && !choice.equals("2"));
				if (!choice.equals("1")) {
					...
					System.out
							.println("Time for which the run will be extended: ");
					do {
						try {
							more_time = (Double.valueOf(input.readLine()))
									.doubleValue();
						} catch (NumberFormatException e) {
						}
						break;
	...
	public static void end_current_run() {
		...
		if (incomplete && (output_analysis_type == BATCH_MEANS)
				&& (!anim_stopped)) {
			...
			if (incomplete && (!anim_stopped)) {
				// In the case of independent replications and an incomplete run
	...
	/**
	 * Internal method used to apply the selected output analysis or variance
	 * reduction technique. This method should <b>not</b> be used in user
	 * simulations.
	 */
	...
	private static void gather_data() {
		...
		run_data[1] = new int[] { trans_condition, term_condition,
				output_analysis_type };
		...
		if ((replications.size() < replication_count) && (!anim_stopped)) {
			...
			if ((term_condition == INTERVAL_ACCURACY) && (!anim_stopped)) {
				...
				for (int i = 0; i < replications_size; i++) {
					...
					Sim_stat stat = ((Sim_stat) ((List) r_data[1])
							.get(term_entity_id));
					double r_mean;
					if (stat.is_efficient(term_measure)) {
						r_mean = stat.average(term_measure);
					} else {
						r_mean = stat.average(term_measure, r_transient_time,
								r_total_time);
					}
					r_means[i] = r_mean;
					total_mean += r_mean;
				...
				total_variance = (total_variance / (replications_size - 1))
						/ replications_size; // s2/n
				// Calculate the total mean standard deviation
				double total_std_dev = Math.sqrt(total_variance);
				double t_value = qt(1.0 - (1.0 - confidence_level) / 2.0,
						replications_size - 1, true);
				...
				if (accuracy >= term_accuracy) {
					...
					replication_count = (int) Math.ceil(replications_size
							* Math.pow(half_width
									/ (term_accuracy * total_mean), 2.0));
					print_message("Performing "
							+ (replication_count - prev_rep_count)
			...
			for (int i = 0; i < entities_size; i++) {
				...
				if (ent_stat == null) {
					...
					for (int j = 0; j < ent_measures_size; j++) {
						...
						String measure = ((String) ((Object[]) ent_measures
								.get(j))[0]);
						Integer measure_type = ((Integer) ((Object[]) ent_measures
								.get(j))[1]);
						int m_type = measure_type.intValue();
						double[] measure_levels = ent_stat.get_levels(measure); // null
																				// if
																				// no
																				// levels
																				// specified
						...
						for (int k = 0; k < replications_size; k++) {
							...
							if (stat.is_efficient(measure)) {
								...
								case Sim_stat.RATE_BASED:
									...
									break;
								default: // Sim_stat.STATE_BASED and
						...
						interval[3] = new Double(total_variance); // The total
																	// mean
																	// variance
						if (total_mean == 0.0) {
							interval[4] = new Double(0.0); // The accuracy
						} else {
							interval[4] = new Double(half_width / total_mean); // The
																				// accuracy
						}
						interval[5] = new Double(total_std_dev); // The total
																	// mean
																	// standard
																	// deviation
						if (!measure.equals("Utilisation")) {
							switch (m_type) {
							case Sim_stat.RATE_BASED:
								interval[6] = new Double(total_count); // The
																		// total
																		// average
																		// count
								break;
							default:
								interval[6] = new Double(total_maximum); // The
																			// total
																			// maximum
								interval[7] = new Double(total_minimum); // The
																			// total
																			// minimum
								if (measure_levels != null) {
						...
						measures.add(new Object[] { measure, measure_type,
								interval });
					}
					confidence_intervals
							.add(new Object[] { ent_name, measures });
				}
			}
			...
			run_data[1] = new int[] { trans_condition, term_condition,
					output_analysis_type };
			run_data[2] = new double[] { total_time_elapsed,
					total_transient_time };
			run_data[3] = initial_seeds;
			run_data[4] = new Double(confidence_level);
			run_data[5] = confidence_intervals; // The total measures and
												// confidence intervals
			run_data[6] = new Integer(replications.size()); // The number of
															// replications
															// performed
			run_data[7] = replications; // The data of each individual
										// replication
			repeat = false;
		}
		return repeat;
	...
	private static boolean batch_means() {
		...
		if ((term_condition == INTERVAL_ACCURACY) && (!anim_stopped)) {
			// This method of simulation termination based on interval accuracy
			// is based on a sequential
			// batch means method proposed by Law and Carson and improved based
			// on the proposals of
			// Schmeiser (1982)
			// Step 2
			Sim_stat stat = ((Sim_entity) entities.get(term_entity_id))
					.get_stat();
			int batch_count = 400;
			...
			for (int i = 0; i < batch_count; i++) {
				...
				double batch_mean = stat.average(term_measure, batch_start,
						batch_end);
			...
			if (s_c > 0.4) {
				...
				print_message("Collecting " + (temp - term_count_bm[1])
						+ " additional observations (Total: " + term_count
						+ ").");
		...
		// Stores for each batch length, the number of times it was considered
		// best for a measure
			...
			if (ent_stat == null) {
				...
				for (int k = 0; k < ent_measures_size; k++) {
					...
					for (int l = min_batches; l <= max_batches; l++) {
						...
						for (int m = 0; m < l; m++) {
							double batch_start = trans_time
									+ (m * batch_length);
							double batch_end = trans_time
									+ ((m + 1) * batch_length);
							double batch_mean = ent_stat.average(measure,
						...
						for (int m = 0; m < l; m++) {
							total_variance += Math.pow(
									(batch_means[m] - total_mean), 2.0);
						}
						total_variance = (total_variance / (l - 1)) / l; // s2/n
						double total_std_dev = Math.sqrt(total_variance);
						double t_value = qt(
								1.0 - (1.0 - confidence_level) / 2.0, l - 1,
								true);
						double half_width = t_value * total_std_dev;
						...
						if (total_mean == 0.0) {
							...
							interval[4] = half_width / total_mean; // The
																	// accuracy
						}
						batch_lengths.add(interval);
						// Check the correlation
						double serial_correlation = serial_correlation(
								total_mean, batch_means);
						if (l == min_batches) {
					...
					measures.add(new Object[] { measure, measure_type,
							batch_lengths });
					// Add one to the batch count that produced the best results
		...
		for (int j = 0; j < confidence_intervals_size; j++) {
			...
			Sim_stat stat = (Sim_stat) ((Sim_entity) entities.get(j))
					.get_stat();
			...
			for (int k = 0; k < measures_size; k++) {
				...
				case Sim_stat.RATE_BASED:
					...
					interval[1] = new Double(best_interval[1]); // The total
																// mean
					interval[2] = new Double(best_interval[2]); // The high
																// bound
					interval[3] = new Double(best_interval[3]); // The total
																// variance
					interval[4] = new Double(best_interval[4]); // The accuracy
					interval[5] = new Double(Math.sqrt(best_interval[3])); // The
																			// total
																			// standard
																			// deviation
					interval[6] = new Double(stat.count(measure, trans_time,
							clock)); // Total count
					break;
				default:
					if (measure_levels != null) {
					...
					interval[5] = new Double(Math.sqrt(best_interval[3])); // The
																			...
																			// deviation
					if (!measure.equals("Utilisation")) {
						interval[6] = new Double(stat.maximum(measure,
								trans_time, clock)); // The maximum
						interval[7] = new Double(stat.minimum(measure,
								trans_time, clock)); // The minimum
						if (measure_levels != null) {
							double[] props = stat.exc_proportion(measure,
									measure_levels, trans_time, clock);
		...
		run_data[6] = new Object[] { new Integer(index + min_batches),
				new Double((clock - trans_time) / (index + min_batches)) };
	...
	private static double serial_correlation(double total_mean,
			double[] batch_means) {
		double r_sum1 = 0.0;
		double r_sum2 = 0.0;
		double mean1 = 0.0;
		...
		for (int m = 0; m < l; m++) {
			if (m < (l - 1)) {
				r_sum1 += (batch_means[m] - total_mean)
						* (batch_means[m + 1] - total_mean);
			}
			if (m < (n - 1)) {
				mean1 += batch_means[m];
			} else {
		...
		for (int m = 0; m < l - 1; m++) {
			if (m < (n - 1)) {
				r1_sum1 += (batch_means[m] - mean1)
						* (batch_means[m + 1] - mean1);
				r1_sum2 += Math.pow(batch_means[m] - mean1, 2.0);
			} else {
				r2_sum1 += (batch_means[m] - mean2)
						* (batch_means[m + 1] - mean2);
	...
	private static void reset() {
		...
		for (int i = 0; i < backup_size; i++) {
			...
			try {
				...
				throw new Sim_exception("Could not reset entity "
						+ ent.get_name() + ".");
			}
			new_ent.reset();
	...
	/**
	 * Internal method used to generate the simulation's report. This method
	 * should <b>not</b> be used in user simulations.
	 */
	public static void generate_report() {
		...
		reporter.append_report("");
		reporter.append_report("Simulation date:         "
				+ df.format(new Date(((long[]) run_data[0])[0])));
		reporter.append_report("Simulation start time:   "
				+ tf.format(new Date(((long[]) run_data[0])[0])));
		reporter.append_report("Simulation end time:     "
				+ tf.format(new Date(((long[]) run_data[0])[1])));
		reporter.append_report("");
		...
		reporter.append_report("############################################################");
		reporter.append_report("");
		reporter.append_report("Total simulated time:    "
				+ ((double[]) run_data[2])[0]);
		reporter.append_report("Total transient time:    "
				+ ((double[]) run_data[2])[1]);
		reporter.append_report("Total steady state time: "
				+ (((double[]) run_data[2])[0] - ((double[]) run_data[2])[1]));
		int trans_condition = ((int[]) run_data[1])[0], term_condition = ((int[]) run_data[1])[1], output_analysis_type = ((int[]) run_data[1])[2];
		String trans_condition_info, term_condition_info, output_analysis_type_name;
		switch (trans_condition) {
		case EVENTS_COMPLETED:
			trans_condition_info = initial_trans_count
					+ " event completions at "
					+ get_entity(trans_entity_id).get_name();
			break;
		case TIME_ELAPSED:
			trans_condition_info = initial_trans_time
					+ " units of elapsed simulated time";
			break;
		case MIN_MAX:
			trans_condition_info = "Truncation based on minimum and maximum observations for "
					+ trans_measure
					+ " of "
					+ get_entity(trans_entity_id).get_name();
			break;
		...
		switch (term_condition) {
		case EVENTS_COMPLETED:
			term_condition_info = initial_term_count + " event completions at "
					+ get_entity(term_entity_id).get_name();
			break;
		case TIME_ELAPSED:
			term_condition_info = initial_term_time
					+ " units of elapsed simulated time";
			break;
		case INTERVAL_ACCURACY:
			term_condition_info = "Confidence interval accuracy for "
					+ term_measure + " of "
					+ get_entity(term_entity_id).get_name();
			break;
		default:
		...
		reporter.append_report("Transient condition:     "
				+ trans_condition_info);
		reporter.append_report("Termination condition:   "
				+ term_condition_info);
		reporter.append_report("Output analysis method:  "
				+ output_analysis_type_name);
		if (output_analysis_type == IND_REPLICATIONS) {
			reporter.append_report("Confidence level:        "
					+ (Double) run_data[4]);
			reporter.append_report("Replications performed:  "
					+ (Integer) run_data[6]);
		} else if (output_analysis_type == BATCH_MEANS) {
			reporter.append_report("Confidence level:        "
					+ (Double) run_data[4]);
			reporter.append_report("Number of batches:       "
					+ (Integer) ((Object[]) run_data[6])[0]);
			reporter.append_report("Individual batch length: "
					+ (Double) ((Object[]) run_data[6])[1]);
		}
		reporter.append_report("");
			...
			if (measures_exist()) {
				...
				for (int i = 0; i < confidence_intervals_size; i++) {
					...
					for (int j = 0; j < measures_size; j++) {
						...
						switch (m_type) {
						case Sim_stat.RATE_BASED:
							reporter.append_report("Total mean:          "
									+ ((Double) measurements[1]).doubleValue());
							reporter.append_report("Interval low bound:  "
									+ ((Double) measurements[0]).doubleValue());
							reporter.append_report("Interval high bound: "
									+ ((Double) measurements[2]).doubleValue());
							reporter.append_report("Interval half width: "
									+ (((Double) measurements[2]).doubleValue() - ((Double) measurements[0])
											.doubleValue()) / 2.0);
							reporter.append_report("Accuracy ratio:      "
									+ ((Double) measurements[4]).doubleValue());
							reporter.append_report("Mean variance:       "
									+ ((Double) measurements[3]).doubleValue());
							reporter.append_report("Mean std deviation:  "
									+ ((Double) measurements[5]).doubleValue());
							reporter.append_report("Average event count: "
									+ ((Double) measurements[6]).doubleValue());
							break;
						default:
							reporter.append_report("Total mean:          "
									+ ((Double) measurements[1]).doubleValue());
							...
							if (!m_name.equals("Utilisation")) {
								reporter.append_report("Total maximum:       "
										+ ((Double) measurements[6])
												.doubleValue());
								reporter.append_report("Total minimum:       "
					...
					for (int j = 0; j < measures_size; j++) {
						...
						case Sim_stat.RATE_BASED:
							if (stat.is_efficient(m_name)) {
								reporter.append_report("Sample mean:          "
										+ stat.average(m_name));
								reporter.append_report("Event count:          "
										+ stat.count(m_name));
							} else {
								reporter.append_report("Sample mean:          "
										+ stat.average(m_name, trans_time,
												total_time));
								reporter.append_report("Event count:          "
										+ stat.count(m_name, trans_time,
												total_time));
							}
							break;
						default:
							if (stat.is_efficient(m_name)) {
								...
								if (!m_name.equals("Utilisation")) {
									...
									reporter.append_report("Maximum:              "
											+ stat.maximum(m_name));
									reporter.append_report("Minimum:              "
											+ stat.minimum(m_name));
									if (levels != null) {
										reporter.append_report("Exceedence proportions:");
										double[] props = stat
												.exc_proportion(m_name);
								...
								reporter.append_report("Sample mean:          "
										+ stat.average(m_name, trans_time,
												total_time));
								if (!m_name.equals("Utilisation")) {
									reporter.append_report("Sample variance:      "
											+ stat.variance(m_name, trans_time,
													total_time));
									reporter.append_report("Sample std deviation: "
											+ stat.std_deviation(m_name,
													trans_time, total_time));
									reporter.append_report("Maximum:              "
											+ stat.maximum(m_name, trans_time,
													total_time));
									reporter.append_report("Minimum:              "
											+ stat.minimum(m_name, trans_time,
													total_time));
									if (levels != null) {
										reporter.append_report("Exceedence proportions:");
										double[] props = stat.exc_proportion(
												m_name, levels, trans_time,
												total_time);
										for (int k = 0; k < levels.length; k++) {
											if (k + 1 == levels.length) {
												reporter.append_report("      "
		...
		if (detailed_report) {
			if (output_analysis_type == IND_REPLICATIONS) {
				...
				for (int i = 0; i < replications_size; i++) {
					...
					reporter.append_report("Steady state time: "
				...
				double total_time = ((double[]) run_data[2])[0];
				double trans_time = ((double[]) run_data[2])[1];
				int batch_count = ((Integer) ((Object[]) run_data[6])[0])
						.intValue();
				double batch_length = ((Double) ((Object[]) run_data[6])[1])
						.doubleValue();
				for (int i = 0; i < batch_count; i++) {
					...
					for (int j = 0; j < stats_size; j++) {
						...
						for (int k = 0; k < measures_size; k++) {
							...
							case Sim_stat.RATE_BASED:
								reporter.append_report("Sample mean:          "
										+ stat.average(m_name, start_time,
												end_time));
								reporter.append_report("Event count:          "
										+ stat.count(m_name, start_time,
												end_time));
								break;
							default:
								reporter.append_report("Sample mean:          "
								...
								if (!m_name.equals("Utilisation")) {
									reporter.append_report("Sample variance:      "
											+ stat.variance(m_name, start_time,
													end_time));
									reporter.append_report("Sample std deviation: "
											+ stat.std_deviation(m_name,
													start_time, end_time));
									reporter.append_report("Maximum:              "
											+ stat.maximum(m_name, start_time,
													end_time));
									reporter.append_report("Minimum:              "
											+ stat.minimum(m_name, start_time,
													end_time));
									if (levels != null) {
										reporter.append_report("Exceedence proportions:");
										double[] props = stat.exc_proportion(
												m_name, levels, start_time,
												end_time);
										for (int l = 0; l < levels.length; l++) {
											if (l + 1 == levels.length) {
												reporter.append_report("      "
	...
	/**
	 * Internal method used to notify <code>Sim_system</code> that the user has
	 * clicked the stop button. This applies to animated simulations and should
	 * <b>not</b> be used in user simulations.
	 */
	...
	/**
	 * Set the desired contents of the trace file. The default is to include all
	 * trace, if tracing is switched on.
	 * 
	 * @param default_trace
	 *            If set to <code>true</code> the default trace will be included
	 *            in the trace file, if <code>false</code> it will not
	 * @param entity_trace
	 *            If set to <code>true</code> the trace generated by the
	 *            entities will be included in the trace file, if
	 *            <code>false</code> it will not
	 * @param event_trace
	 *            If set to <code>true</code> the trace file will include trace
	 *            for the selected event types, if <code>false</code> it will
	 *            not
	 */
	public static void set_trace_detail(boolean default_trace,
			boolean entity_trace, boolean event_trace) {
		auto_trace = true;
	...
	static synchronized void trace(int src, String msg) {
		trcout.println("u: " + ((Sim_entity) entities.get(src)).get_name()
				+ " at " + clock + ": " + msg);
	}
	...
	private static void generate_graphs() {
		...
		try {
			...
			System.out.println("Unable to generate graph data: "
					+ ioe.getMessage());
		} finally {
			if (output != null) {
				try {
	...
	// Called by Sim_stat objects to notify Sim_system that an efficient measure
	// has been defined
	static synchronized void efficient_measure_defined() {
		efficient_measure_defined = true;
	}

	// Informs Sim_stat objects of the transient time end. This is used to reset
	// any efficient measures
