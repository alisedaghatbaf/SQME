 ...
 * Only changes made to this population can be tracked; changes made directly to
 * the contained solutions will not be detected. Therefore, it may be necessary
...
public class NondominatedSortingPopulation extends Population {
	...
	public NondominatedSortingPopulation(FastNondominatedSorting sorting) {
		super();
		modified = false;
		fastNondominatedSorting = sorting;
	}
	 ...
	 * {@link #truncate(int)}, except the crowding distance is recalculated each
	 * time a solution is removed.
	 * 
	 * @param size
	...
	public void prune(int size) {
		...
		int maxRank = (Integer) super.get(size - 1)
				.getAttribute(RANK_ATTRIBUTE);
	 ...
	 * population. This method will in general be called automatically when the
	 * population is modified. However, only changes made to this population can
	 * be tracked; changes made directly to the contained solutions will not be
	 * detected. Therefore, it may be necessary to invoke {@link #update()}
	 * manually.
	 */
	public void update() {
		modified = false;
